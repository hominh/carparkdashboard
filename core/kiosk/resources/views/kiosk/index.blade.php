...
@section('page')
    <link rel="stylesheet" type="text/css" href="{{asset('css/kiosk/style.css')}}">
        ...
        <script>
            $(function() {
                $('#listKiosk').on('click', '.deleteDialog', function (e) {
                    e.preventDefault();
                    var url = '{{ url("/kiosk/destroy", ":id") }}';
        ...
        <script>
            var htmlstr = '<input type="checkbox" id="checkAllKiosk" class="table-check-all" data-set=".dataTable .checkboxes" /> ';
            $('.msg').html(htmlstr);
        </script>
        <script>
            $(function() {
                $('#checkAllKiosk').change(function(){
                ...
                $(".deleteKiosk").on("click", function(){
                    var ids = [];
                    $('#listKiosk').find('tr').each(function () {

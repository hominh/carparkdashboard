...
@section('page')
    <link rel="stylesheet" type="text/css" href="{{asset('css/basement/style.css')}}">
        ...
        <script>
            $(function() {
                $('#listBasement').on('click', '.deleteDialog', function (e) {
                    e.preventDefault();
                    var url = '{{ url("/basement/destroy", ":id") }}';
        ...
        <script>
            var htmlstr = '<input type="checkbox" id="checkAllBasement" class="table-check-all" data-set=".dataTable .checkboxes" /> ';
            $('.msg').html(htmlstr);
        </script>
        <script>
            $(function() {
                $('#checkAllBasement').change(function(){
                ...
                $(".deleteBasement").on("click", function(){
                    var ids = [];
                    $('#listBasement').find('tr').each(function () {
                    ...
                    else
                    {
                        ...
                        if(check == true){
                            ...
                            var url = '{{ url("/basement/multi-destroy") }}';
                            $.ajax({
                                url: url,
                                type: 'DELETE',
                                ...
                                success: function (data) {
                                    if(data.status == 'success')
                                        toastr.success("Xóa hầm giữ xe thành công");
                                else
                                        toastr.error("Xóa hầm giữ xe thất bại");
                                window.LaravelDataTables["listBasement"].ajax.reload();
                                },
                                error: function (error) {
                                    if(error.status == 403)
                                        window.LaravelDataTables["listBasement"].ajax.reload();
                                        toastr.error("Error: 403. Xóa hầm giữ xe thất bại");
                                    }
                                });
                            }
                        }
